...
public class Function4FifthTest extends Function implements IFunction {
    private double a, b, c;

    public Function4FifthTest(double left, double right, double a, double b, double c) throws IncorrectDomainException {
        super(left, right);
        this.a = a;
        this.b = b;
        this.c = c;
    ...
    public double compute(double x) throws OutOfSegmentException, IncorrectDomainException, ArithmeticException, OutOfDomainException, OutOfSegmentException, IncorrectDomainException {
        ...
        {
            return a * Math.sin(b * x * x) + c * Math.exp(x);
        }
    }
}
